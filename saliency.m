...
imgRoot='./saliency_map/test/';% test image path
saldir= './saliency_map/saliencymap/';% the output path of the saliency map
supdir= './saliency_map/superpixels/';% the superpixel label file path

if 7==exist(saldir,'dir')
    rmdir(saldir, 's'); % removing the test folder
end
if 7==exist(supdir,'dir')
    rmdir(supdir, 's'); % removing the test folder 
end
mkdir(supdir);
mkdir(saldir);
